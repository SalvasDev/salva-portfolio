...
export default function Project() {
  ...
  const found = dbProjects.find(
    (element) => element?.name === project && element?.area === platform
  );
        ...
        <div className="detail__container center">
          <section className="detail__info center">
            <div className="detail__data animate__animated animate__fadeInLeft">
              ...
              <h3 className="detail__area">
                {(found?.area || "").charAt(0).toUpperCase() +
                  (found?.area || "").slice(1)}
              </h3>
              <h3 className="detail__element">Diseño: {found?.designed}</h3>
              {found?.developed && (
                <h3 className="detail__element">
                  Desarrollo: {found?.developed}
                </h3>
              )}
              <h4 className="detail__share">
                Compartir en:{" "}
                <a
                  href={`https://www.facebook.com/sharer/sharer.php?u=https://odissy.vercel.app/${found?.area}/${found?.name}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  FB
            ...
            <div>
              <div
                className="description animate__animated animate__fadeInRight"
                dangerouslySetInnerHTML={htmlContent}
              />
              <div className="projects__labels">
                <a href={found?.code} target="_blank" rel="noopener noreferrer">
                  <button className="projects__btn">Código</button>
                </a>
                <a href={found?.demo} target="_blank" rel="noopener noreferrer">
            ...
            {(found?.pics || []).map((img) => {
              ...
              return (
                <>
                  <div
                    key={ind.toString()}
                    className="detail__image center animate__animated animate__fadeInUp"
                  >
                    <Image
                      className="detail__pic center"
                      width="1920"
                      height="1280"
                      src={img}
                      alt="Imágenes del proyecto"
                    />
                  </div>
                </>
              );
          ...
          {showVerticalBtnDetail && (
            <ButtonDetail smart__class="vertical__arrowbtn-detail" />
          )}
        </div>
      </BasicLayout>
    </>
